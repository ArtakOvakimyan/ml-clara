...
def run_inference(checkpoint: str, stage: int, question: str, document: str, max_tokens: int):
    ...
    #from checkpoints_mem64.stage2.modeling_clara import CLaRa
    from openrlhf.models.modeling_clara import CLaRa
    #from modeling_new import CLaRa
    ...
    with torch.no_grad():
        if stage == 1:
            ...
            # model_rag = AutoModel.from_pretrained(checkpoint, trust_remote_code=True, pure_inference=True)
            # model = model_rag.to(device)
            # model.eval()
...
def main():
    ...
    parser.add_argument("--question", default="Как называют быстроходные суда на подводных крыльях?")
